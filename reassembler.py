...
class Instruction(object):
    ...
    def assembly(self, comments=False, symbolized=True):
        ...
        if not symbolized:
            ...
            asm = not_symbolized
			#print(type(self.insn))
            #print("[oprand id] %s %d"%(self.op_str, self.insn.id))
            #print(asm)
            #print([ operand.symbolized for operand in self.operands ])
            
            #print(type(self.insn)) angr.block.CapstoneInsn
            #print(self.insn.mnemonic, self.insn.id)
            #print(type(self.operands[0]))
            '''
            ins_obf = Obfuscated(self.insn,self.addr)
